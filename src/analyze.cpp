...
static Error resolve_union_zero_bits(CodeGen *g, ZigType *union_type) {
    ...
    for (uint32_t i = 0; i < field_count; i += 1) {
        ...
        if (decl_node->type == NodeTypeContainerDecl) {
            ...
            union_field->is_zero_bits = false;
            ...
            if (field_node->data.struct_field.type == nullptr) {
                ...
                if ((err = type_val_resolve_zero_bits(g, field_type_val, union_type, nullptr, &union_field->is_zero_bits))) {
                    union_type->data.unionation.resolve_status = ResolveStatusInvalid;
                    return ErrorSemanticAnalyzeFail;
                }
            }
        ...
        if (create_enum_type) {
            ...
            AstNode *tag_value = union_field->decl_node->data.struct_field.value;
    ...
    if (decl_node->type == NodeTypeContainerDecl && create_enum_type) {
